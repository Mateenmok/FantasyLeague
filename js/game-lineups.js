(() => {
  ...
  const edit=({game,home,away,lineup,catalog,rosters,onApply})=>{
    ...
    show(`Edit Game ${game}`,`<p>Select up to four Pokémon per side. Leave unrevealed Pokémon unselected — even zero is fine.</p><div class="game-lineup-sides">${['home','away'].map(side=>{
    ...
    update();dialog.querySelector('.game-lineup-apply').onclick=()=>{onApply({game,home:choices.home,away:choices.away});dialog.close();};
  };
