...
async function loadPokemonDatabase() {
  const [pokemonJson, championsJson] = await Promise.all([
    fetchJsonSafe("data/pokemon.json?v=profile-favorites1"),
    fetchJsonSafe("data/champions-pokemon.json?v=profile-favorites1")
  ]);

  const fullPokemon = flattenPokemonData(pokemonJson);
  const championsPokemon = flattenPokemonData(championsJson);
  ...
  for (const raw of fullPokemon) {
    ...
    if (normalized.name) {
      fullByName.set(normalizeNameKey(normalized.name), normalized);
    }
  }

  const combined = [];

  for (const raw of championsPokemon.length ? championsPokemon : fullPokemon) {
    const normalized = normalizePokemon(raw);
    const backup = fullByName.get(normalizeNameKey(normalized.name));

    if (backup) {
      normalized.image = normalized.image || backup.image;
    }
  ...
  if (!combined.length) {
    for (const raw of fullPokemon) {
      const normalized = normalizePokemon(raw);
      if (normalized.name) combined.push(normalized);
    }
  }
...
function saveFavorite(index, requestedName) {
  const statusEl = document.getElementById("favoritePokemonStatus");
