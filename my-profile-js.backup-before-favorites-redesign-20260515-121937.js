...
async function initProfile() {
  ...
  const profileInfoBar = document.getElementById("profileInfoBar");
  const profileAccountNoteEl = document.getElementById("profileAccountNote");
  ...
  if (!client) {
    ...
    profileInfoBar.textContent = "Supabase client was not found.";
    return;
  }
  ...
  if (sessionError) {
    ...
    profileInfoBar.textContent = "There was a problem loading your session.";
    return;
  }
  ...
  if (userIdEl) userIdEl.textContent = `User ID: ${(user.id || "unknown").slice(0, 8)}...`;
  if (miniStatusEl) miniStatusEl.textContent = "Account loaded";
  profileInfoBar.textContent = `${username} • ${email}`;
  if (profileSubtitleEl) profileSubtitleEl.textContent = username;
  if (profileAccountNoteEl) profileAccountNoteEl.textContent = "Connected account profile";
  ...
  uploadInput.addEventListener("change", async (event) => {
    ...
    try {
      ...
      if (avatarTypeEl) avatarTypeEl.textContent = "Avatar: Custom";
...
async function loadAvatar({ avatarImg, avatarFallback, avatarTypeEl, statusEl, avatarKey, npcKey }) {
  ...
  if (savedNpc && await imageExists(savedNpc)) {
    ...
    if (avatarTypeEl) avatarTypeEl.textContent = "Avatar: Default";
    statusEl.textContent = "";
    return;
  }
