...
function openLeaguemateProfile(teamId) {
  ...
  const logoUrl = team.logo_url || "";
  const roleLabel = team.is_admin ? "Admin" : "Manager";
  const npcUrl = getPersistentNpcForTeam(team);
  ...
  const changeNpcButton = document.getElementById("changeLeagueNpc");

  if (changeNpcButton) {
    changeNpcButton.addEventListener("click", function () {
      changeNpcForTeam(team);
    });
  }
...
function getPersistentNpcForTeam(team) {
  const key = getNpcStorageKeyForTeam(team);
  const savedNpc = localStorage.getItem(key);

  if (savedNpc && LEAGUEMATE_NPC_IMAGES.includes(savedNpc)) {
    return savedNpc;
  }

  const assignedNpc = getStableNpcForTeam(team);
  localStorage.setItem(key, assignedNpc);
  return assignedNpc;
}

function changeNpcForTeam(team) {
  const key = getNpcStorageKeyForTeam(team);
  const currentNpc = localStorage.getItem(key) || getPersistentNpcForTeam(team);
  const currentIndex = LEAGUEMATE_NPC_IMAGES.indexOf(currentNpc);
  const nextIndex = currentIndex >= 0
    ? (currentIndex + 1) % LEAGUEMATE_NPC_IMAGES.length
    : 0;

  localStorage.setItem(key, LEAGUEMATE_NPC_IMAGES[nextIndex]);
  openLeaguemateProfile(team.id);
}

function getChangeNpcButtonHtml(team) {
  const teamManagerEmail = String(team.manager_email || "").toLowerCase();
  const isOwnTeam = currentUserEmail && teamManagerEmail && currentUserEmail === teamManagerEmail;

  if (!isOwnTeam) {
    return "";
  }

  return `<button class="league-profile-secondary" id="changeLeagueNpc">Change NPC</button>`;
}

function getNpcStorageKeyForTeam(team) {
  const leaguePart = String(selectedLeagueId || "league").replace(/[^a-zA-Z0-9_-]/g, "");
  const teamPart = String(team.id || team.team_number || team.team_name || "team").replace(/[^a-zA-Z0-9_-]/g, "");

  return `pokeleague_leaguemate_npc_${leaguePart}_${teamPart}`;
}
